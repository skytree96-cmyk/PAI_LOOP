...
## 2026-09-04 추가 — 실행 검증으로 결론 갱신

`docs/codex-handoff/2026-09-04/codex_prompt_quantitative_root_cause_verified.md`를
**먼저** 읽을 것. 어제 문서와 충돌하면 그 문서가 우선한다. 핵심 갱신:
- PR #69는 필요조건일 뿐 충분조건이 아님(실증). 진짜 병목은 **신용등급 범위 표현**.
- 부산 20/20은 `_BUSAN_*` 공고 특화 상수에 의존(어제 "미확인" → 확정).
- 재현 스크립트 2개 동봉(회귀 canary).
