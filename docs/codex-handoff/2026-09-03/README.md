...
4. **`codex_prompt_quantitative_scoring_fix.md`** — 정량점수 수정 작업 지시서
   (Phase 1~4). **전부 미착수** — Codex가 처음부터 진행. (2026-09-03 재검증으로
   줄번호·Phase 3 범위·Phase 4 근거가 정정됨, PR #71 참고)
5. **`codex_prompt_quantitative_display_fix.md`** — 정량점수 **표출(화면 반영)**
   관련 Phase 5. Phase 1~4(산정)와 별개로, "계산은 맞는데 화면엔 안 보이는"
   리스크 2건을 다룸. **전부 미착수.**
...
| 항목 | 상태 | 담당 |
|---|---|---|
| 자격요건 프레시니스 버그 수정 (Phase 1) | ✅ 완료, PR #70 병합됨, 테스트 통과 | Claude (완료) |
...
| 공동수급 지분율 로직 | ✅ 유닛 레벨 구현·테스트 완료 확인됨 (재검증), 파이프라인 연결만 확인 필요 | Codex |
| n8n W11 COMPLETED 판정 기준 변경 | ⬜ 미착수, API 스키마 확장 선행 필요 (재검증으로 확정) | Codex |
| 정량점수 표출(공개 화면 캐시 미스) 리스크 수정 (Phase 5) | ⬜ 미착수, 신규 발견 | Codex |
