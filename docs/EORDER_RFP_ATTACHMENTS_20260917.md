...
- 차수는 **앞의 0 을 떼고** 비교한다(`_revision_key`). 공고 목록은 차수를 두 자리로
  채우고(`normalise_notice`) 전자주문은 세 자리로 준다. 문자열로 그대로 맞추면
  제공자가 자릿수를 바꾸는 순간 조인이 **조용히 전부** 실패한다.

#### 조인 키 실측 (2026-09-01 ~ 09-17)
...
놓친 3건은 공고가 차수 000 인데 제안요청서가 001 로 재발급된 경우다. 공고번호만으로
조인하면 3건을 더 얻지만, **다른 차수의 제안요청서를 붙이게 된다.** 지금 코드는
근거를 공고 버전에 묶는 것을 일관되게 지키므로 보수적인 쪽을 택했다. 0.7% 는 그 대가다.
