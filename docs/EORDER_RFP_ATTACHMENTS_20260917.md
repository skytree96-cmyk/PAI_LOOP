...
- **기본값은 꺼짐이다.** `PAI_LOOP_EORDER_RFP_ATTACHMENTS_ENABLED=true` 로 켠다.
  3.3 의 재추출 비용을 알고 켜야 하는 항목이라, 배포만으로 저절로 켜지지 않게 했다.
  5장의 순서를 지킬 수 있는 것도 이 플래그 덕분이다.
...
기회를 잃고 비용이 한꺼번에 나간다. 순서는 이렇다.

1. 이 PR 을 병합·배포한다. 플래그가 꺼져 있으므로 **동작은 하나도 바뀌지 않는다.**
2. 소수 공고로 통제된 확인을 한다.
3. 결과를 보고 `PAI_LOOP_EORDER_RFP_ATTACHMENTS_ENABLED=true` 로 켠다.
4. 일일 워크플로가 나머지를 따라잡는다.
