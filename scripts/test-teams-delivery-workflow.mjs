...
assert.deepEqual(scheduled.parameters.rule.interval.map((item) => item.expression), ["30,45 8 * * *", "*/15 9 * * *", "0,15 10 * * *"]);
...
assert.equal(scheduledFinal.schedule.cron, "30,45 8 * * *; */15 9 * * *; 0,15 10 * * *");
...
console.log("Teams 08:30 readiness gate, bounded 15-minute retries through 10:15, stable daily correlation, manual-live separation, sanitizer, persistent reservation, and native sink contracts passed.");
