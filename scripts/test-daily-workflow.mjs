...
assert.equal(runtime.maxBacklogRetryNotices, 3);
...
assert.match(nodes.get("Reserve or Resume Daily Analysis Operation").parameters.body, /max_total: 3003/);
...
const ppsResponse = {
  ...
  window: { from: "2026-08-16", to: "2026-08-17" }, dry_run: false,
  keywords_used: ["교육", "컨설팅", ...Array.from({ length: 24 }, (_, index) => `부서-${index}`)],
  provider_queries: 26, department_coverage_count: 24, api_calls: 26, fetched: 7,
...
const rootPlan = one(nodes, "Build Bounded Batch Analysis Plan", { awardRefresh: { status: "COMPLETED" } }, {
  node: {
    ...
    "Fetch Award Candidates from Seven-Day Briefing": { json: { analysis_queue: { policy: "NEVER_ATTEMPTED_THEN_OLDEST_RETRY", pending_total: 9, notice_keys: ["backlog-1", "backlog-2", "backlog-3", "backlog-4"], never_attempted_notice_keys: ["backlog-1", "backlog-2"], retryable_notice_keys: ["backlog-3", "backlog-4"] } } },
  },
});
...
assert.deepEqual(rootPlan.analysisBatch.backlogKeys, ["backlog-1", "backlog-2", "backlog-3"]);
assert.deepEqual(rootPlan.analysisBatch.neverAttemptedBacklogKeys, ["backlog-1", "backlog-2"]);
assert.deepEqual(rootPlan.analysisBatch.retryableBacklogKeys, ["backlog-3"]);
assert.equal(rootPlan.analysisBatch.retryEpoch, "2026-08-17");
...
assert.deepEqual(rootPlan.analysisBatch.noticeKeys.slice(-3), ["backlog-1", "backlog-2", "backlog-3"]);
...
assert.equal(manifest.workflows["pai-loop-11-analysis-backfill"].publish, false);
assert.equal(manifest.workflows["pai-loop-11-analysis-backfill"].promotionState, "awaiting-live-e2e");
