    ...
    def _extract(
        ...
        *,
        ...
        source_prompt = (
            ...
            "that subtotal, and never bind it to the whole proposal score. "
            ...
            "A scoring table keeps the condition and its award in separate cells, and both belong in "
            "the literal. criterion_literal must carry the row's own max_points as the source writes "
            "it (수행실적 20점, not 수행실적), and each bracket.literal and case.literal must carry "
            "that row's own award (5건 이상 9점, not 5건 이상). When the award sits in an adjacent "
            "cell, transcribe the cells together in source order into both the literal and its "
            "evidence.quote; a cell boundary is not a reason to drop the number. Keep a bare condition "
            "cell verbatim (4건, 3인) and set the structured bounds to that exact value on both sides. "
            ...
            "reverse 이상/초과/이하/미만 or >=/>/<=/<. Use BRACKET for continuous numeric rows "
            "with both printed endpoints, and for ascending, non-overlapping numeric interval "
            "tables; these are not descending overlapping CASE_TABLE cutoffs. For example, "
            "20% 이상 40% 미만 2점 has min_value=20, max_value=40, min_inclusive=true, "
            "max_inclusive=false and points=2. Preserve both printed endpoints; never encode "
            "this bounded row as only GTE 20. For explicit closed count ranges in BRACKET, "
            "including personnel counts, 2~4명 3점 has min_value=2, max_value=4, "
            "min_inclusive=true, max_inclusive=true and points=3; the same closed-range "
            "transcription applies to an explicit 2~4인 or 2~4건 row. Do not discard its upper "
            "endpoint or replace the range with a single lower cutoff. Preserve BRACKET array "
            "order and CASE_TABLE row_order in printed source order; never reverse or sort "
            "source rows to make a different execution method pass validation. An unprinted "
            "upper bound must remain absent: never borrow a boundary from another row or "
            "criterion, and never invent a complementary band. Preserve the source metric and "
            "unit: a percent interval alone does not make a criterion FINANCIAL_RATIO, and "
            "must never be converted to KRW. Use UNKNOWN when no supported metric fits exactly; "
            "unsupported or ambiguous semantics must remain for review. For a known metric, required_evidence must "
            ...
            + ". Use CASE_TABLE for source-ordered descending overlapping GTE cutoffs, exact discrete "
            "rows, rating/category groups, or percentage-of-maximum rows when their conditions "
            "form a supported ordered case program. Do not turn an ascending disjoint interval "
            "table into lower-bound-only CASE rows. Preserve source row order "
            ...
            "Within an otherwise valid source-ordered PERFORMANCE_COUNT CASE_TABLE, transcribe "
            "explicit inclusive ranges (for example, 5건∼6건 4.5점) using "
            "BETWEEN with comparison_value=5, comparison_upper_value=6 and empty category_values. "
            "Do not replace that valid BETWEEN program merely because a row has "
            "two endpoints. "
            ...
            "a literal 배점 as POINTS and a percentage such as 배점의 95% as PERCENT_OF_MAX. "
            "The BRACKET interval guidance does not authorize converting a percentage-of-maximum "
            "award into a literal points award or dropping a valid CASE program. Never "
