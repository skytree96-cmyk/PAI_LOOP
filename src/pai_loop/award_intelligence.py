...
# year, and the ordinal/half markers that ride alongside them.
_TITLE_YEAR_PATTERN = re.compile(
    r"(?:\b(?:19|20)\d{2}\s*년도?|\b(?:19|20)\d{2}\b|['\u2019]\d{2}\s*년?|"
    r"\b제?\s*\d+\s*(?:차|기|회)\b|\b(?:상|하)반기\b)"
)
_TITLE_NOISE_PATTERN = re.compile(r"[\s\(\)\[\]{}<>·ㆍ,./\\|_\-~!@#$%^&*+=:;\"\'`?]+")
...
def normalise_project_title(value: Any) -> str:
    ...
    Removing the year, the ordinal and the punctuation is what lets the 2024,
    2025 and 2026 editions of one recurring project match each other. It is a
    normalisation, not a similarity judgement: two titles either reduce to the
    same string or they do not.
    """

    text = str(value or "")
    without_year = _TITLE_YEAR_PATTERN.sub(" ", text)
    return _TITLE_NOISE_PATTERN.sub("", without_year).casefold()


def _normalise_agency(value: Any) -> str:
    return _TITLE_NOISE_PATTERN.sub("", str(value or "")).casefold()
...
def _opening_companies(row: Any) -> list[dict[str, Any]] | None:
    value = _value(row, "opening_results")
    if value is None:
        return None
    ...
    for entry in value:
        if not isinstance(entry, Mapping):
            continue
        name = str(entry.get("company_name") or "").strip()
        if not name:
            continue
        companies.append({
            "company_name": name,
            "bid_amount": _number(entry.get("bid_amount")),
            "technical_evaluation": _number(entry.get("technical_evaluation")),
            "price_evaluation": _number(entry.get("price_evaluation")),
            "total_evaluation": _number(entry.get("total_evaluation")),
            "opening_rank": _participant_count(entry.get("opening_rank")),
        })
    return companies


def _participation_kind(company_name: str, winner_name: str) -> str:
    """Decide winner vs participant from the award endpoint, never from rank.

    The opening endpoint orders companies by opening result; the award
    endpoint is what names a winner. With no stored winner there is nothing to
    compare against, so the row stays UNKNOWN instead of promoting rank 1.
    """

    if not winner_name:
        return "UNKNOWN"
    return "WINNER" if _normalise_agency(company_name) == _normalise_agency(winner_name) else "PARTICIPANT"


def build_annual_award_table(
    records: Iterable[Any],
    *,
            ...
            entries = [
                {
                    **base,
                    **company,
                    "participation_kind": _participation_kind(company["company_name"], winner_name),
                }
    ...
    def sort_key(item: dict[str, Any]) -> tuple[Any, ...]:
        return (
            -(item["year"] or 0),
            0 if item["participation_kind"] == "WINNER" else 1,
            -(item["total_evaluation"] if item["total_evaluation"] is not None else -1),
            item["company_name"],
        )
    ...
    if not_collected:
        notes.append(
            f"{not_collected}건은 개찰 결과를 아직 조회하지 않아 참여업체와 평가점수가 없습니다."
        )
    if empty_openings:
        notes.append(f"{empty_openings}건은 개찰 결과 조회에서 사용 가능한 업체 행이 없었습니다.")
    if undated:
        notes.append(f"{undated}건은 개찰·낙찰 일자가 없어 연도를 확정하지 못했습니다.")
    if rows and not scored_rows:
        notes.append("현재 저장본에는 기술·가격·종합 평가점수가 없습니다.")

    return {
        "table_version": ANNUAL_AWARD_TABLE_VERSION,
        "generated_as_of": reference.isoformat(),
        "years": years,
        ...
        "opening_results_not_collected": not_collected,
        "rows": rows,
        "notes": notes,
    }
