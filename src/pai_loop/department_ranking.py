...
def load_department_keyword_profiles() -> dict[str, Any]:
    ...
    required_root = {"version", "baseline", "departments"}
...
def rank_notice_for_department(
    *,
    ...
    searchable_text = _normalize(" ".join((title, agency, category)))
    ...
    department_score = round(
        sum(
            ...
            if item["source"] in {"DEPARTMENT_STRONG", "DEPARTMENT_SUPPORTING", "REGION"}
        ),
        1,
    )
    ...
    return {
        ...
        "ranking_scope": "REGION" if department and department.get("group") == "지역그룹" else "BUSINESS",
        "score": score,
        "raw_score": raw_score,
...
def rank_notice_across_departments(
    *,
    ...
    """Return departments with actual department/region matches, best first."""
    ...
    differentiated = [item for item in rankings if item["department_score"] > 0]
    differentiated.sort(
        key=lambda item: (
