...
def test_table_and_card_actions_keep_detail_view_and_label_direct_result_entry() -> None:
    ...
    assert "styles.css?v=20260914-quantitative-gapclass-v1" in html
    assert "top-navigation.css?v=20260913-dashboard-charts-v2" in html
    assert "app.js?v=20260914-quantitative-gapclass-v1" in html
...
def test_manual_analysis_actions_are_functional() -> None:
    ...
    assert 'if (state.manualAnalysisRequests.has(noticeKey)) return' in source
    assert 'state.manualAnalysisRequests.get(noticeKey) === flight' in source
