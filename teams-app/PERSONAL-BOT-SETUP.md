...
- 연결 콜백은 코드 내용으로 답장하지 않으며 어떤 응답 본문도 돌려주지 않습니다.
  연결이 성립하면 워커가 그 사람의 부서 기준 브리핑 한 건을 예약하고, 그 메시지가
  연결 완료를 대신 알립니다. 콜백 안에서 보내는 것이 아니므로 잘못된 코드에는
  아무 일도 일어나지 않습니다. 이미 연결된 사람에게만 가므로 새로 드러나는 정보는
  없고, 같은 날 다시 연결해도 브리핑은 한 번만 갑니다.
- 관심 공고를 등록하면 그 공고의 알림이 별도로 예약됩니다. 브리핑과 관심 공고
  알림은 같은 개인 대화로 가지만 서로 독립적으로 예약·발송됩니다.
